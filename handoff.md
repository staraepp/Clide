...
- **Still not started:** text animations (word-stagger reveal on headlines, matching the site's `split-heading` hero treatment) and frosted/liquid glass (`.ultraThinMaterial`/`.regularMaterial` on the floating pill, with genuine macOS 26 `.glassEffect()` as a possible later `#available`-gated upgrade). These are the two remaining pieces of the original "10000000x cooler" ask — see "What's next" below.
...
2. **Finish the "10000000x cooler" ask that prompted the shader work** — one of three pieces done:
